...
class Network(object):
    ...
    def conv2d(self, inputs, kernel_size, stride, out_channels, name=None, padding='SAME'):
        ...
        return tf.nn.conv2d(input=inputs, filters=kernel, strides=strides, padding=padding)
    ...
    def split_conv2d(self, inputs, kernel_size, rate, out_channels, name=None, padding='SAME', both_atrous=False):
        ...
        if both_atrous:
            ...
            outA = tf.nn.conv2d(input=inputs, filters=kernelA, strides=strides, padding=padding)
            outB = tf.nn.atrous_conv2d(inputs, kernelB, rate, padding=padding)
    ...
    def batch_norm(self, inputs):
        ...
        with tf.compat.v1.variable_scope('BatchNorm'):
            gamma = tf.compat.v1.get_variable('gamma', (in_channels,), initializer=tf.compat.v1.constant_initializer(1.0),
                                    trainable=self.training, dtype=self.float_type,
                                    regularizer=tf.keras.regularizers.l2(l=0.5 * (self.weight_decay)))
            beta = tf.compat.v1.get_variable('beta', (in_channels,), initializer=tf.compat.v1.constant_initializer(0),
                                   trainable=self.training, dtype=self.float_type,
                                   regularizer=tf.keras.regularizers.l2(l=0.5 * (self.weight_decay)))
            moving_mean = tf.compat.v1.get_variable('moving_mean', (in_channels,),
                                          initializer=tf.compat.v1.constant_initializer(0), trainable=False, dtype=self.float_type)
            moving_var = tf.compat.v1.get_variable('moving_variance', (in_channels,),
                                         initializer=tf.compat.v1.constant_initializer(1), trainable=False, dtype=self.float_type)

        if self.training:
            batch_mean, batch_var = tf.nn.moments(x=inputs, axes=[0, 1, 2])
            train_mean = tf.compat.v1.assign(moving_mean, moving_mean* self.bn_decay_ + batch_mean * (1 - self.bn_decay_))
            train_var = tf.compat.v1.assign(moving_var, moving_var * self.bn_decay_ + batch_var * (1 - self.bn_decay_))
    ...
    def pool(self, x, k_size, strides, name=None, padding='SAME'):
        return tf.nn.max_pool2d(input=x, ksize=[1, k_size, k_size, 1], strides=[1, strides, strides, 1], padding=padding)
    ...
    def conv_batchN_relu(self, x, kernel_size, stride, out_channels, name, relu=True, dropout=False):
        with tf.compat.v1.variable_scope(name):
            ...
            if dropout:
                conv_out = tf.nn.dropout(conv_out, 1 - (self.keep_prob))
    ...
    def unit_4(self, x, out_channels, block, unit, shortcut=False, dropout=False):
        if shortcut:
            with tf.compat.v1.variable_scope('block%d/unit_%d/bottleneck_v1/conv3'%(block-1, 6)):
                x_bn = tf.nn.relu(self.batch_norm(x))
        else:
            with tf.compat.v1.variable_scope('block%d/unit_%d/bottleneck_v1/conv3'%(block, unit-1)):
                x_bn = tf.nn.relu(self.batch_norm(x))
        ...
        with tf.compat.v1.variable_scope('block%d/unit_%d/bottleneck_v1'%(block, unit)):
            ...
            if dropout:
                b3_u3_1e_cout=tf.nn.dropout(b3_u3_1e_cout, 1 - (self.keep_prob))
    ...
    def fc(self, inputs, out_channels, name):
        ...
        with tf.compat.v1.variable_scope(name): 
            b = tf.compat.v1.get_variable('biases', [out_channels], trainable=self.training,
                                initializer=tf.compat.v1.constant_initializer(0.01), dtype=self.float_type)
