...
def test_func(config):
    ...
    prediction_numpy = np.empty([1,384,768], np.int64)
    ...
    while 1:
        try:
            ...
            prediction_numpy = np.append(prediction_numpy,prediction,axis=0)
            #print(img)
            #print(img.shape)
            #print(prediction)
            #print(prediction.shape)
        except tf.errors.OutOfRangeError:
            np.save('image_rgb.out', image_numpy)
            np.save('prediction_rgb.out', prediction_numpy)
            break
